
# Leap (Final Major Project)

This project is a 2D Fast-paced Side-scroller Platformer built using the Unity Engine. It was developed as part of my Computer Science Extended Diploma qualification to demonstrate core gameplay mechanics, level design, implementation and any other additional bonuses

## Features

- Smooth Player movement with responsive jumping, double jumping, and sliding mechanics
- Enemy AI and Obstacles
- Rank Players based on coins collected and completion time
- Interactable Power-ups
- Weather Elements & World Building with each new Level



## Tech Stack

- Unity Engine (2D)
- C#
- Tilemaps
- DOTween
- Asynchronous & Coroutine

## Controls

- Jumping: Space
- Double Jumping: Space 2X
- Sliding: Left Shift

## How to Run the Game
```
1.	Create a New Unity Project
	•	Open Unity Hub → New Project → 2D template
	•	Select the same Unity version as this project
	•	Name the project and create it
```
```
2.	Replace Project Files
	•	Close Unity
	•	Go to the project folder (Windows: C:/Users/.../Unity Projects/YourProjectName/, Mac: /Users/.../Unity Projects/YourProjectName/)
	•	Delete everything inside
	•	Copy all files from this repository into the folder (replace Assets, ProjectSettings, Packages)
```
```
3.	Open the Project
	•	Open Unity Hub → Open → select the project folder
	•	Wait for Unity to load the project
```
```
4.	Play the Game
	•	Open the main scene (Assets/Scenes/)
	•	Press the Play button in the Editor
	•	Or Having the Option to Build the game and play it full Screen
```

## Promotional Image

![App Screenshot](https://github.com/user-attachments/assets/db7381e1-f75a-42d6-8b4e-b04bceaf9b11)


## Main Menu

![App Screenshot](https://raw.githubusercontent.com/SaeedAAli/L3-Extended-Diploma-Creative-Media-Y2-2024-2025/main/ezgif.com-video-to-gif-converter.gif)



## Level 1

Level Location: Green Zone Park

![App Screens](https://raw.githubusercontent.com/SaeedAAli/L3-Extended-Diploma-Creative-Media-Y2-2024-2025/main/level1gifrepoleap-ezgif.com-video-to-gif-converter.gif)


## Level 2

Level Location: Industrial Zone

![App Screenshot](https://raw.githubusercontent.com/SaeedAAli/L3-Extended-Diploma-Creative-Media-Y2-2024-2025/main/level2gifrepoleap-ezgif.com-video-to-gif-converter.gif)


## Licenses & Third Party Assets Used

Tilesets & Player Animation Used

 - [kainshiro](https://kainshiro.itch.io/pixel-art-basic-parkoursurvival-movement-pack)

-  [Dead Revolver](https://deadrevolver.itch.io/pixel-prototype-player-sprites)

-  [Craftpix](https://craftpix.net/freebies/free-industrial-zone-tileset-pixel-art/)

- [Craftpix](https://craftpix.net/freebies/free-green-zone-tileset-pixel-art/?num=1&count=132&sq=green&pos=3)


User Interface 

- [Google Fonts](https://fonts.google.com/specimen/Rock+3D?preview.text=Preview%20Text)

- [DOTween](https://assetstore.unity.com/packages/tools/animation/dotween-hotween-v2-27676)

- [Keybinds](https://assetstore.unity.com/packages/2d/gui/icons/game-input-controller-icons-free-285953)


- [Pause Menu](https://assetstore.unity.com/packages/2d/gui/icons/simple-free-pixel-art-styled-ui-pack-165012)


In Game Assets

- [RainDrop](https://i.ibb.co/qgfgynJ/rainDrop.png)

- [Coin Pickup](https://laredgames.itch.io/gems-coins-free)

- [Destroyable Platform](https://www.piskelapp.com/)

- [BouncePad](https://www.piskelapp.com/)

- [Main Menu Background](https://assetstore.unity.com/packages/2d/environments/night-mountain-lake-seamless-127703)



Sound Effects

- [Main Menu Music](https://www.piskelapp.com/)

- [Game Soundtrack for All Levels](https://www.piskelapp.com/)

- [Coin Pickup Sound Effect](https://www.piskelapp.com/)

- [Player Footsteps](https://www.piskelapp.com/)

- [Player Land](https://pixabay.com/sound-effects/land2-43790/)

- [Player SpeedBoost](https://mixkit.co/free-sound-effects/discover/boost/)

- [Jump & Double Jump](https://pixabay.com/sound-effects/8-bit-jump-001-171817/)

- [Collision Sound Effect](https://pixabay.com/sound-effects/collapse-47652/)

